...
async function catMsg(recvhash) {
    ...
    let time_recv = Date.now();
    console.log(`Sent: ${jsonmsg.time}`);
    // insertMessageToDOM(jsonmsg, false);
    console.log(`Recv: ${time_recv}`);
    time_log.push({ msg: jsonmsg.content, time_sent: jsonmsg.time, time_recv: time_recv, time_taken: time_recv - jsonmsg.time });
}
...
var headers = {
    msg: "Message",
    time_sent: "Time_Sent",
    time_recv: "Time_Received",
    time_taken: "Time_Taken"
};

function downloadLog() {
    exportCSVFile(headers, time_log, 'time-log');
}
...
function sendM() {
    ...
    var interval = setInterval(function() {
        var obj = lines[i];
        document.querySelector('input[type="text"]').value = obj.msg;
